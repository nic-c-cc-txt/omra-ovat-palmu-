...
function hitAntiDdos(value) {
    if (value === null) {
        return false;
    }
    return (/^<!DOCTYPE.*>/gi).test(value.trim());
}
exports.hitAntiDdos = hitAntiDdos;
function getList(path, keepCurrent) {
    return __awaiter(this, void 0, void 0, function* () {
        let data = yield http_1.getData(`${config_1.API_URL}/list`);
        if (hitAntiDdos(data)) {
            data = yield http_1.getData(`${config_1.ALTERNATIVE_API_URL}/list`);
        }
        if (data === null) {
            return null;
        }
