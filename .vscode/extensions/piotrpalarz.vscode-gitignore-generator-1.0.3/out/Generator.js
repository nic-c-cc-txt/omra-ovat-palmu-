...
class Generator {
    ...
    generate() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let data = yield http_1.getData(`${config_1.API_URL}/${this.selected.join(",")}`);
            if (helpers_1.hitAntiDdos(data)) {
                data = yield http_1.getData(`${config_1.ALTERNATIVE_API_URL}/${this.selected.join(",")}`);
            }
            if (data === null) {
                return vscode_1.window.showErrorMessage(config_1.MESSAGES.network_error);
            }
